const Snapshots = require("../model/snapshots");

const uploadImage = async (rqe, res) => {
  const { testId, studentId, image } = req.body;
  let snapshots = await Snapshots.findByIdAndUpdate(
    { testId, studentId },
    { $push: { images: image } }
  );
  ...
  if (!snapshots) {
    const images = [];
    images.push(image);
    snapshots = new Snapshots({ testId, studentId, images });
